//
// StockTrackerData.h
//
// Copyright (c) 2020-2025 Larry M. Taylor
//
// This software is provided 'as-is', without any express or implied
// warranty. In no event will the authors be held liable for any damages
// arising from the use of this software. Permission is granted to anyone to
// use this software for any purpose, including commercial applications, and to
// to alter it and redistribute it freely, subject to 
// the following restrictions:
//
// 1. The origin of this software must not be misrepresented; you must not
//    claim that you wrote the original software. If you use this software
//    in a product, an acknowledgment in the product documentation would be
//    appreciated but is not required.
// 2. Altered source versions must be plainly marked as such, and must not be
//    misrepresented as being the original software.
// 3. This notice may not be removed or altered from any source
//    distribution.
//

#import <Foundation/Foundation.h>

#import "LTPopup.h"
#import "LTLog.h"

@interface StockTrackerData : NSObject
{
    // For data fetching
    NSURLSessionDataTask *mPriceDataTask;
    NSURLSessionDataTask *mCompanyDataTask;
    NSURLSessionDataTask *mCompanyNewsTask;
    NSData *mPriceData;
    NSData *mCompanyData;
    NSData *mNewsData;
    NSTimer *mFetchTimer;
    
    // For popup window
    LTPopup *mPopupWindow;
    NSMutableString *mText;

    // For logging
    os_log_t mLog;
}

@property NSString *symbol;
@property NSString *companyName;
@property NSString *news;
@property double currentPrice;
@property double change;
@property double pricePaid;
@property double numberOfShares;
@property double gain;
@property double gainPercent;
@property double dividends;

+ (void)setApiKey:(NSString *)apiKey;
+ (void)setLogFile:(NSString *)logFile;

- (id)initWithSymbol:(NSString *)symbol withPricePaid:(double)pricePaid
    withNumberOfShares:(double)numberOfShares;
- (void)updateData;

@end
